...
const tableList = () => {
  ...
  getUserList(searchFormData).then((res: any) => {
    console.log(res)
...
const handleLock = (index: any, row: any) => {
  let is_lock = row.is_lock === 1 ? 2 : 1
  postUserLock(row.id, is_lock).then((res: any) => {
    row.is_lock = is_lock
  })
}
...
const handleDelete = (index: any, row: any) => {
  ElMessageBox.confirm('确定删除该用户？').then(() => {
    postUserDel(row.id).then((res: any) => {
      tableData.value.splice(index, 1)
    })
  })
}

const handleOpenVip = (index: any, row: any) => {
  row.index = index
  userRow.info = row
...
<template lang="pug">
el-container.my-table-container
  el-form.my-form
    ...
    el-form-item(label="昵称:")
      el-input(v-model="searchFormData.nick_name" placeholder="Please input nick_name")
    ...
    el-form-item(label="注册时间范围:")
      el-date-picker(v-model="searchFormData.created_at_start"  type="datetime" placeholder="Please Select start time" value-format="YYYY-MM-DD HH:mm:ss")
      el-date-picker(v-model="searchFormData.created_at_end"  type="datetime" placeholder="Please Select end time" value-format="YYYY-MM-DD HH:mm:ss")
    el-form-item
  ...
  el-table(:data="tableData" height="70vh" style="width: 100%")
    ...
    el-table-column(prop="vip" label="vip等级")
      template(#default="scope")
        div {{ vipConfig[scope.row.vip].Name }}
    el-table-column(prop="user_contact.superior" label="上级UID")
    el-table-column(prop="wallet" label="钱包")
      template(#default="scope")
        div 总收益: {{ scope.row.wallet.total_balance }}
        div 余额: {{ scope.row.wallet.balance }}
    el-table-column(prop="vip_end_at" label="VIP到期时间")
    el-table-column(prop="created_at" label="注册时间")
    el-table-column(prop="" label="操作")
      template(#default="scope")
        el-row
          el-button(size="small" type="primary" @click="handleOpenVip(scope.$index, scope.row)") 开通VIP
        el-row
          el-button(size="small" type="primary" @click="handleLock(scope.$index, scope.row)") {{ scope.row.is_lock === 1 ? "锁定" : "解除锁定" }}
        el-row
          el-button(size="small" type="danger" @click="handleDelete(scope.$index, scope.row)") 删除
  el-pagination(background layout="total, sizes, prev, pager, next, jumper" v-model:page-size="pageSize" v-model:currentPage="currentPage" :page-sizes="[10, 30, 50, 100]" :total="total")
