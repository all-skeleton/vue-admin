...
const onOpenVip = (res: any) => {
  ...
  postUserOpenVip(oData).then((res: any) => {
    loading.close()
    if (res.code === 1) {
...
el-dialog(v-model="showOpenVipDialog" title="开通VIP")
  el-form.my-edit(label-width="5rem")
    el-form-item(label="用户信息:") {{ userRow.info.nick_name }}: {{ userRow.info.user_name }}
    el-form-item(label="当前等级:") {{ vipConfig[userRow.info.vip].Name }}
    el-form-item(label="开通等级:")
      el-select(v-model="oData.level" placeholder="选择VIP等级")
        el-option(v-for="item in vipConfig" :label="item.Name" :value="item.Level")
          span 等级:{{ item.Name }} (价格:{{ item.Price }} 收益:{{item.Profit}}% 时长:{{ item.Length === -1 ? "无限制" : item.Length + "月" }} )
    el-form-item(label="支付金额:")
      el-input(v-model="oData.show_price")
    el-form-item(label="备注:")
      el-input(v-model="oData.remark" :rows="2" type="textarea" placeholder="Please input")
    el-form-item
      el-button(type="primary" @click="onOpenVip") 提交
</template>
